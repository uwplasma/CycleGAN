...
def memory_report(prefix=""):
    ...
    print(f"[Rank {rank}] {prefix}. Memory: {mem_MB:.2f} MB", flush=True)
...
def process_equilibrium(i, eq_relpath, scalar_features, scalar_feature_matrix, FSA_grad_xs, fixed_data, varied_data):
    ...
    try: qi = np.sum(QuasiIsodynamicResidual(stel, [1/16, 5/16, 9/16, 13/16])**2)
...
def main():
    ...
    for i in iterator:
        ...
        except Exception as e: print(f"[Rank {rank}] ERROR at index {i}: {e}", flush=True)
        gc.collect()
        stel_ind += 1
    current, peak = tracemalloc.get_traced_memory()
    if rank==0: print(f"[Rank {rank}] Final memory usage: {current / 10**6:.2f} MB; Peak: {peak / 10**6:.2f} MB", flush=True)
    tracemalloc.stop()
